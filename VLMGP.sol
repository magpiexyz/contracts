...
contract VLMGP is IVLMGP, Initializable, ERC20Upgradeable, OwnableUpgradeable, PausableUpgradeable, ReentrancyGuardUpgradeable {
    ...
    function totalLocked() override public view returns (uint256) {
        return totalSupply() - this.totalAmountInCoolDown();
    }
    ...
    function startUnlock(uint256 _amountToCoolDown) public override whenNotPaused nonReentrant {
        if (_amountToCoolDown > getUserTotalLocked(msg.sender))
            revert NotEnoughLockedMPG();

        IMasterMagpie(masterMagpie).depositVlMGPFor(0, msg.sender); // just for claiming reward
    ...
    function unlock(uint256 _slotIndex) external override whenNotPaused nonReentrant {
        ...
        _withdrawAndBurn(unlockedAmount);
    ...
    function forceUnLock(uint256 _slotIndex) external whenNotPaused nonReentrant {
        ...
        _withdrawAndBurn(slot.amountInCoolDown);
    ...
    function _withdrawAndBurn(uint256 _unlockedAmount) internal {
        IMasterMagpie(masterMagpie).withdrawVlMGPFor(_unlockedAmount, msg.sender);
        _burn(address(this), _unlockedAmount);          
    }
